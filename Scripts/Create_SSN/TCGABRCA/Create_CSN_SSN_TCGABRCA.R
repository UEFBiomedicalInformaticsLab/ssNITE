...
# Script to prepare SSN using CSN (Cell-specific network) for TCGA-BRCA samples
# Notes:
...
if(!dir.exists("IOFiles/Create_SSN/TCGABRCA/CSN/")){dir.create("IOFiles/Create_SSN/TCGABRCA/CSN/", recursive = TRUE)}
...
network <- create_CSN_net( gene_expr = rnaSeq_vst,
                           csn_weighted = TRUE,
                           IO_path = "IOFiles/Create_SSN/TCGABRCA/CSN/",
                           n_cores = 11,
                           use_slurm = TRUE )

write_parquet( x = network,
               sink = "IOFiles/Create_SSN/TCGABRCA/CSN/CSN_SSN__DA_TCGA__DT_BreastCancer.parquet",
               compression = "lz4" )
print(paste0(Sys.time(), " |   -Done!"))

system("rm -r IOFiles/Create_SSN/TCGABRCA/CSN/CSN_edges_tmp/")
gc()
