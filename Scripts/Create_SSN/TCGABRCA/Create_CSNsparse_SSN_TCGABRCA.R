...
# Script to prepare SSN using CSN (Cell-specific network) for TCGA-BRCA samples [sparse mode]
# Notes:
# Refer: Cell-specific network constructed by single-cell RNA sequencing data (https://doi.org/10.1093/nar/gkz172) 
...
if(!dir.exists("IOFiles/Create_SSN/TCGABRCA/CSNsparse/")){dir.create("IOFiles/Create_SSN/TCGABRCA/CSNsparse/", recursive = TRUE)}
...
#   tmp1 <- tmp1 %>% sample(ceiling(length(tmp1) * 0.01))
...
source("Scripts/Functions/CSN/Functions_CSN__main.R")

network <- create_CSN_net( gene_expr = rnaSeq_vst,
                           csn_weighted = FALSE,
                           IO_path = "IOFiles/Create_SSN/TCGABRCA/CSNsparse/",
                           n_cores = 11,
                           use_slurm = TRUE )

write_parquet( x = network,
               sink = "IOFiles/Create_SSN/TCGABRCA/CSNsparse/CSNsparse_SSN__DA_TCGA__DT_BreastCancer.parquet",
               compression = "lz4" )
print(paste0(Sys.time(), " |   -Done!"))

system("rm -r IOFiles/Create_SSN/TCGABRCA/CSNsparse/CSN_edges_tmp/")
gc()
